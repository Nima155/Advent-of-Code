...
pub fn generate_3d_moves() -> [[i8; 4]; 80] {
    let mut ans = [[0; 4]; 80];
    ...
    grid.iter()
        ...
        .map(|(i, subgrid)| {
            subgrid
                ...
                .filter_map(|(j, c)| {
                    if let '#' = *c {
                        Some((i as i32, j as i32, 0_i32, 0_i32))
                    } else {
                        None
                    }
                })
...
pub fn node_cycle(actives: &HashSet<Point>) -> HashSet<Point> {
    ...
    for (y, x, z, m) in actives.iter() {
        let mut count = 0;
        for [i, j, v, c] in moves {
            let new_point = (*y + i as i32, *x + j as i32, *z + v as i32, *m + c as i32);
            if actives.contains(&new_point) {
                count += 1;
            } else {
                let mut bad_count_actives = 0;
                for [i_bad, j_bad, v_bad, c_bad] in moves {
                    let new_point_inactive = (
                        new_point.0 + i_bad as i32,
                        new_point.1 + j_bad as i32,
                        new_point.2 + v_bad as i32,
                        new_point.3 + c_bad as i32,
                    );
