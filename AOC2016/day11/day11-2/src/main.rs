...
fn main() {
        ...
        .map(|e| {
            ...
            for item in temp_v {
                if item.contains("generator") {
                    ...
                    bit_set |= 1 << (ELEMENTS.iter().position(|e| *e == ele).unwrap() + 7)
                } else {
                    let ele = item.split('-').next().unwrap();
    ...
    println!("{}", search_and_move([vc[0], vc[1], vc[2], vc[3]], 7));
...
fn is_valid_combination(floors: [i64; 4], shift: i64) -> bool {
    for i in floors {
        for j in 0..shift {
            if (i & (1 << j)) != 0 && (i & 0b11111110000000) != 0 && ((i & (1 << (j + shift))) == 0)
            {
                return false;
            }
        }
