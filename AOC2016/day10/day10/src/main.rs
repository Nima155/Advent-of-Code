...
macro_rules! instrcution_filler {
    ($maps: expr, $number: expr,  $instructions: expr, $instructions_of: expr) => {{
        let bucket = $maps.entry($instructions_of).or_insert([-1, -1]);
        if $instructions_of != -1 {
            if bucket[0] == -1 {
                bucket[0] = $number;
            } else {
                bucket[1] = $number;
                process(($maps, $instructions), $instructions_of);
            }
        }
    }};
}
    ...
    for l in lines {
        ...
        if l.starts_with("value") {
            ...
            entry.push(Instruction {
                high_to: if split_line[10] != "output" { last } else { -1 },
                low_to: if split_line[5] != "output" {
                    split_line[6].parse::<i64>().unwrap()
                } else {
                    -1
                },
            });
...
fn process(
    ...
    if low != -1 && high != -1 && instructions.contains_key(&cur_bot) {
        ...
        instrcution_filler!(maps, low, instructions, instructions_of.low_to);
        instrcution_filler!(maps, high, instructions, instructions_of.high_to);
    }
}
