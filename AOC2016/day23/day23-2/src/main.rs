...
fn execute_program(instructions: &mut [Vec<&str>]) -> i64 {
    ...
    while i >= 0 && i < instructions.len() as i64 {
        ...
        // println!("instruction: {:?} registers: {:?}", split_up, registers);
        match split_up[0] {
            "mul" => {
                let fr;
                if let Ok(first) = split_up[1].parse::<i64>() {
                    fr = first;
                } else {
                    fr = registers[get_indx(split_up[1])];
                }

                registers[0] = fr * registers[get_indx(split_up[2])];
            }
            "cpy" => {
