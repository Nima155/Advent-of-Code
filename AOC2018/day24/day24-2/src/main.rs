...
fn main() {
    ...
    for i in 0.. {
        let mut boosted_battlefield = battlefield_durum_durum.clone();
        for b in boosted_battlefield.iter_mut() {
            if b.5 == false {
                b.3.0 += i;
            }
        }
        let res= play_the_game(boosted_battlefield);
        if res != 0 {
            println!("{}", res);
            break;
        }
...
fn play_the_game(mut battlefield_durum_durum: Vec<Stats>) -> i64 {
    loop {
        let bef = battlefield_durum_durum.clone();
        ...
        if battlefield_durum_durum.iter().all(|s| s.5 == gr) {
            ...
            return battlefield_durum_durum.iter().map(|s| if s.5 == false { s.0 } else { 0 }).sum();
        }
        else if battlefield_durum_durum == bef {
            return 0;
        }
    }
}
