...
fn test_input(cur_data: &str) -> bool {
    ...
    let initial_registers = initial_registers[1..initial_registers.len() - 1].split(", ").map(|c| c.parse::<i64>().unwrap()).collect::<Vec<_>>();
    let after_registers = unparsed_data[2].split(":  ").collect::<Vec<_>>()[1];
    let after_registers = after_registers[1..after_registers.len() - 1].split(", ").map(|c| c.parse::<i64>().unwrap()).collect::<Vec<_>>();

    let operations = unparsed_data[1].split(' ').map(|c| c.parse::<i64>().unwrap()).collect();
