...
int is_anagram(const char *l, const char *r) {
    int buff[26] = { 0 };
    int buff_2[26] = { 0 };
    int i = 0;

    for (; i < strlen(l); i++) buff[l[i] - 'a']++;

    for (i = 0; i < strlen(r); i++) buff_2[r[i] - 'a']++;
...
int check_validity(char *words[]) {
    ...
    for (int i = 0; i < MAX_WORDS; i++) {
        ...
        for (int j = 0; j < MAX_WORDS; j++) {
            ...
            if (i != j && words[i] != NULL && words[j] != NULL && is_anagram(words[i], words[j]) == 1) {
                return 0;
            }
            ans = 1;
        }
