...
void main_logic(char *instructions[], char *conditions[]) {
...
void free_and_find_max(int *mx) {
    Node *current, *tmp;

    HASH_ITER(hh, map, current, tmp) {
        *mx = *mx > *current->value ? *mx: *current->value;
        ...
        if (any(instructions, INSTRUCTION_LENGTHS) || any(conditions, INSTRUCTION_LENGTHS)) {
            ...
            while ((c = fgetc(f_handle)) != EOF) {
                if (c == '\n') {
                    ...
                    i_indx = 0;
                    c_indx = 0;
                    main_logic(instructions, conditions);
                    i = 0;
                }
