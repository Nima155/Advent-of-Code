...
fn main() {
    let connections = fs::read_to_string("../input.txt").unwrap();
...
fn bfs(connections: &HashMap<&str, Vec<&str>>, connection_indices: &[&str]) -> usize {
    ...
    while !queue.is_empty() {
        let (on, visited, double_whammy) = queue.pop_front().unwrap();
        ...
        for neigh in &connections[on] {
            ...
            let is_big = neigh.chars().next().unwrap().is_uppercase();
            if (1 << indx_of & visited) == 0 || double_whammy == 0 || is_big {
                if !is_big && (1 << indx_of) & visited != 0 && *neigh != "end" && *neigh != "start"
                {
                    queue.push_back((neigh, visited, double_whammy | (1 << indx_of)));
                } else if is_big || (1 << indx_of) & visited == 0 {
                    queue.push_back((neigh, visited | (1 << indx_of), double_whammy));
                }
            }
        }
    }
