...
fn find_beacons(scanners_and_rots: &[Vec<Vec<Vec<i64>>>]) -> i64 {
    ...
    for p_start in &composite_scanner {
        for p_start_1 in &composite_scanner {
            if p_start != p_start_1 {
                largest_dist = i64::max(manhattan_distance(&p_start.0, &p_start_1.0), largest_dist);
            }
        }
    }
    largest_dist
