...
const ROTATIONS: [[i8; 2]; 4] = [[0, 1], [1, 0], [0, -1], [-1, 0]];

pub fn run_the_program(program: &mut HashMap<usize, i64>) -> HashMap<(i32, i32), bool> {
    let (mut instruction_pointer, mut output_flag) = (0, -1);

    let (mut relative_base, mut cur_pos, mut cur_rot) = (0, (0, 0), 3_usize);

    let mut painted_panels: HashMap<(i32, i32), bool> = HashMap::new();

    painted_panels.insert(cur_pos, true);
            ...
            || (cur_instruction < 10 && is_valid_op(cur_instruction))
        {
            ...
            match cur_instruction {
                ...
                3 => {
                    program.insert(
                        parameter as usize,
                        *painted_panels.get(&cur_pos).or(Some(&false)).unwrap() as i64,
...
pub fn build_grid(painted_tiles: &HashMap<(i32, i32), bool>) {
    let (max_y, max_x) = (painted_tiles.keys().map(|f| f.0).max().unwrap(), painted_tiles.keys().map(|f| f.1).max().unwrap());

    let mut grid = vec![vec![0; max_x as usize + 1]; max_y as usize + 1];

    for (coord, stat) in painted_tiles {
        if *stat {
