...
fn run_the_program(program: &mut HashMap<usize, i64>) -> i64 {
    ...
    let mut ot_total = String::new();
    let mut input_store = VecDeque::new();
    loop {
            ...
            || (cur_instruction < 10 && is_valid_op(cur_instruction))
        {
            ...
            match cur_instruction {
                // 203 too low
                4 | 104 => {
                    let ot_to_char = char::from_u32(if cur_instruction == 104 {
                        parameter
                    } else {
                        *program.entry(parameter as usize).or_default()
                    } as u32).unwrap();
                    ot_total.push(ot_to_char);
                    print!("{}", ot_to_char);
                    if ot_total.ends_with("Command?") {
                        let mut str: String = String::new();
                        std::io::stdin().read_line(&mut str).unwrap();
                        input_store.extend(format!("{}\n", str.trim()).chars());
                    }
                }
                204 => {
                    let ot_to_char = char::from_u32(*program
                        .entry((parameter + relative_base) as usize)
                        .or_default() as u32).unwrap();
                    ot_total.push(ot_to_char);
                ...
                3 => {
                    program.insert(parameter as usize, input_store.pop_front().unwrap() as i64); // change to 2 for answer to part 2
                }

                203 => {
                    program.insert((parameter + relative_base) as usize,  input_store.pop_front().unwrap() as i64); // change to 2 for answer to part 2
                }
