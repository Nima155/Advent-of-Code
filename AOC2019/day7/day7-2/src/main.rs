...
fn main() {
    ...
    for comb in (5..10).permutations(5) {
        let mut next_first = 0;
        let mut caches = Vec::new();
        'a: loop {
            for (indx, j) in comb.iter().enumerate() {
                if caches.len() < 5 {
                    let ans = run_the_program(&mut program, *j, next_first, 0);
                    caches.push((program.clone(), ans.2));
                    next_first = ans.1
                } else {
                    let ins_pointer = caches[indx].1;
                    let ans = run_the_program(&mut caches[indx].0, -1, next_first, ins_pointer);
...
fn run_the_program(
    ...
    let mut instruction_pointer = first_pointer;
    ...
    while instruction_pointer < program.len() {
            ...
            || (program[instruction_pointer] < 10 && is_valid_op(program[instruction_pointer]))
        {
            ...
            match program[instruction_pointer] {
                3 => {
                    program[parameter as usize] = if first_inp != -1 {
                        first_inp
                    } else {
                        second_input
                    };
                ...
                4 => {
                    return (false, program[parameter as usize], instruction_pointer + 2);
                }
                _ => {}
            }
