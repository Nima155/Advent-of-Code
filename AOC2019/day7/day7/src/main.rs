...
fn main() {
    ...
    for comb in (0..5).permutations(5) {
        let mut next_first = 0;
        for j in comb {

            next_first = run_the_program(&mut program, j, next_first);
        }
...
fn run_the_program(program: &mut Vec<i64>, mut first_inp: i64, second_input: i64) -> i64 {
    ...
    while instruction_pointer < program.len() {
            ...
            || (program[instruction_pointer] < 10 && is_valid_op(program[instruction_pointer]))
        {
            ...
            match program[instruction_pointer] {
                3 => {
                    program[parameter as usize] = if first_inp != -1 { first_inp } else { second_input };
                    first_inp = -1;
                }
