...
fn run_the_program(program: &mut Vec<i64>) -> i64 {
    ...
    while instruction_pointer < program.len() {
        ...
        if (is_valid_instruction(program[instruction_pointer]) && (last_digit == 1 || last_digit == 2)) || (program[instruction_pointer] == 1 || program[instruction_pointer] == 2) {
            // println!("{}", program[instruction_pointer]);
            let modes = extract_parameter_modes(program[instruction_pointer]);

            let (mut indx_dest, mut indx_f1, mut indx_f2) = (
                program[instruction_pointer + 3] ,
                program[instruction_pointer + 1] ,
                program[instruction_pointer + 2] ,
            );
            
            indx_f1 = if modes.is_empty() || *modes.back().unwrap() == 0 { program[indx_f1 as usize] } else {indx_f1};
            indx_f2 = if modes.len() < 2 || modes[modes.len() - 2] == 0 { program[indx_f2 as usize] } else {indx_f2};
            // indx_dest = if *modes.front().unwrap() == 0 { program[indx_dest as usize] } else {indx_dest};
            

            match last_digit {
                1  => {
                    program[indx_dest as usize] = indx_f1 + indx_f2;
                }
                2 => {
                    program[indx_dest as usize] = indx_f1 * indx_f2;
                }
            ...
            match program[instruction_pointer] {
                3 => { program[parameter as usize] = 1; },
                4 => { println!(" {}", program[parameter as usize]) },
                99 => { break; }
                _ => {}
            }
