...
fn main() {
    ...
    let mut program = read
        ...
        run_the_program(&mut program, ["NOT A T", "NOT B T", "NOT C T", "AND D T", "NOT A J", "OR T J", "WALK"].iter().map(|s| {
            let mut veco = s.chars().map(|c| c as u8).collect::<VecDeque<_>>();
            veco.push_back(b'\n');
            veco
        }).flatten().collect::<VecDeque<_>>());
    ...
    loop {
            ...
            || (cur_instruction < 10 && is_valid_op(cur_instruction))
        {
            ...
            match cur_instruction {
                ...
                203 => {
                    program.insert((parameter + relative_base) as usize, inputs.pop_front().unwrap() as i64); 
                }
